...
- 기존 **NAIS3와 같은 데이터 폴더**(`%APPDATA%\NAIS3`)를 사용합니다 — 쓰던 씬·캐릭터·폴더·프롬프트·설정·토큰이 **그대로 이어집니다.** 따로 옮길 필요 없어요.
- 이 때문에 **진짜 공식 NAIS3와 동시에 쓰는 건 권장하지 않습니다** (데이터를 공유하므로). 커스텀판만 쓰시면 됩니다.
