...
const toOutfit = (r: Row): Outfit => ({
  id: r.id,
  name: r.name,
  pieces: normalizePieces(r.pieces),
  negative: r.negative ?? ''
})
...
export function listOutfits(): Outfit[] {
  return (
    getDb()
      .prepare('SELECT id, name, pieces, negative FROM outfits ORDER BY sort_order, id')
      .all() as Row[]
  ).map(toOutfit)
}

export function getOutfit(id: number): Outfit | null {
  const r = getDb().prepare('SELECT id, name, pieces, negative FROM outfits WHERE id = ?').get(id) as
    | Row
    | undefined
  return r ? toOutfit(r) : null
}

export function createOutfit(name: string, pieces: OutfitPiece[] = [], negative = ''): number {
  const db = getDb()
  const max = (db.prepare('SELECT COALESCE(MAX(sort_order), 0) AS m FROM outfits').get() as { m: number }).m
  return Number(
    db
      .prepare('INSERT INTO outfits (name, pieces, negative, sort_order) VALUES (?, ?, ?, ?)')
      .run(name.trim() || '새 복장', JSON.stringify(normalizePieces(pieces)), negative, max + 1)
      .lastInsertRowid
  )
}
...
export function updateOutfit(
  id: number,
  patch: { name?: string; pieces?: OutfitPiece[]; negative?: string }
): void {
  const sets: string[] = []
 ...
 */
export function findOrCreateOutfit(name: string, pieces: unknown, negative = ''): number {
  const clean = normalizePieces(pieces)
  const label = name.trim() || '새 복장'
  const same = getDb()
    .prepare('SELECT id FROM outfits WHERE name = ? AND pieces = ? AND negative = ?')
    .get(label, JSON.stringify(clean), negative) as { id: number } | undefined
  return same?.id ?? createOutfit(label, clean, negative)
}
...
export function importOutfits(list: unknown): Map<string, number> {
  ...
  for (const o of list as { uid?: unknown; name?: unknown; pieces?: unknown; negative?: unknown }[]) {
    ...
    map.set(
      ...
      findOrCreateOutfit(
        ...
        o.pieces,
        typeof o.negative === 'string' ? o.negative : ''
      )
    )
  }
  return map
...
export function exportOutfits(
  ids: Iterable<number>
): { uid: string; name: string; pieces: OutfitPiece[]; negative?: string }[] {
  const out: { uid: string; name: string; pieces: OutfitPiece[]; negative?: string }[] = []
  for (const id of new Set(ids)) {
    ...
    if (o)
      out.push({
        ...
        name: o.name,
        pieces: o.pieces,
        ...(o.negative.trim() ? { negative: o.negative } : {})
      })
  }
  return out
}
...
export function duplicateOutfit(id: number): number | null {
  const o = getOutfit(id)
  return o ? createOutfit(`${o.name} 복사`, o.pieces, o.negative) : null
}
